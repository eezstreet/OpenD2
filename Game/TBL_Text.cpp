...
tbl_handle TBL_Register(char* szTblFile)
{
	...
	{
		if (!D2_stricmp(TBLFiles[i].szHandle, szTblFile))
		{
			return i;
		}
	}

	D2_strncpyz(pTBL->szHandle, szTblFile, 16);
	...
	pTBL->archiveHandle = FSMPQ_FindFile(szPathStr, nullptr, &pTBL->pArchive);
	if (pTBL->archiveHandle == INVALID_HANDLE)
	{	// couldn't find this TBL file
		return INVALID_HANDLE;
	}
	pTBL->dwFileSize = MPQ_FileSize(pTBL->pArchive, pTBL->archiveHandle);
	...
	MPQ_ReadFile(pTBL->pArchive, pTBL->archiveHandle, pFileBuffer, pTBL->dwFileSize);
	...
	{
		Log_Print(PRIORITY_MESSAGE, "Couldn't load TBL %s: wrong version (got %i, expected %i)",
	...
	for (int i = 0; i < pTBL->header.HashTableSize; i++)
	{
		...
		{	
			...
			// Read key
			if (!D2_stricmp(GAME_CHARSET, "LATIN"))
			{	// if we're on latin charset, we need to convert these from multibyte to widechar
				D2_qmbtowc(pTBL->pDataNodes[i].key, MAX_TBL_KEY_SIZE, (char*)pReadHead + pHash->StringKeyOffset);
			}
			else
			{
				D2_qstrncpyz(pTBL->pDataNodes[i].key, 
			...
			if (!D2_stricmp(GAME_CHARSET, "LATIN"))
			{
				numRead = D2_qmbtowc(pTBL->szStringTable + strTableRead, 
					pHash->StringValLength, 
					(char*)pReadHead + pHash->StringValOffset);
			}
			else
			{
				numRead = D2_qstrncpyz(pTBL->szStringTable + strTableRead,
...
void TBL_Debug(tbl_handle tbl)
{
	...
	for (int i = 0; i < pTBL->header.NodesNumber; i++)
	{
		...
		D2_qwctomb(key, MAX_TBL_KEY_SIZE, pNode->key);
		D2_qwctomb(str, 8096, pNode->str);
 ...
 */
static char16_t* TBL_TryFindStringInternal(tbl_handle tbl, tbl_handle dwIndex)
{
	TBLFile* pTBL = &TBLFiles[tbl];
...
char16_t* TBL_FindStringFromIndex(DWORD dwIndex)
{
	...
	{
#ifdef EXPANSION
		pReturnValue = TBL_TryFindStringInternal(gExpansionStrings, dwIndex - TBL_EXPANSION_OFFSET);
	...
	{
		pReturnValue = TBL_TryFindStringInternal(gPatchStrings, dwIndex - TBL_PATCH_OFFSET);
		if (pReturnValue != nullptr)
		{
			return pReturnValue;
		}
	...
	{
		pReturnValue = TBL_TryFindStringInternal(gBaseStrings, dwIndex - TBL_CLASSIC_OFFSET);
		if (pReturnValue != nullptr)
		{
			return pReturnValue;
		}
	...
	{
		pReturnValue = TBL_TryFindStringInternal(i, dwIndex);
		if (pReturnValue != nullptr)
		{
			return pReturnValue;
		}
...
{
	TBLFile* pTBL = &TBLFiles[tbl];
	fs_handle hash = D2_qstrhash(szReference, MAX_TBL_KEY_SIZE, pTBL->header.HashTableSize);
	DWORD dwHashTries = 0;
	...
	while (dwHashTries < pTBL->header.HashMaxTries)
	{
		...
		{
			if (!D2_qstrcmp(pTBL->pDataNodes[hash].str, szReference))
			{
				return hash;
			}
		}
...
char16_t* TBL_FindStringText(char16_t* szReference)
{
	...
	for (int i = gnLastUsedTBL - 1; i >= 0; i--)
	{
		tbl_handle x = TBL_FindStringIndexFromKey(i, szReference);
		if (x != INVALID_HANDLE)
		{
			return TBL_TryFindStringInternal(i, x);
		}
	}
...
void TBL_Init()
{
	gBaseStrings = TBL_Register("string");
	gExpansionStrings = TBL_Register("expansionstring");
	gPatchStrings = TBL_Register("patchstring");

	// uncomment if you are curious as to what's in these files
	//TBL_Debug(gBaseStrings);
	//TBL_Debug(gExpansionStrings);
	//TBL_Debug(gPatchStrings);
}
